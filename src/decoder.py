...
class ConvTransE(torch.nn.Module):
    def __init__(self, num_entities, num_rel, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=50, kernel_size=3, use_bias=True):
        ...
        self.bn3 = torch.nn.BatchNorm1d(embedding_dim)
        self.bn_init = torch.nn.BatchNorm1d(embedding_dim)
        self.gate = GatingMechanism(num_entities, num_rel, embedding_dim)

    def forward(self, embedding, emb_rel, triplets, his_emb, pre_weight, pre_type, partial_embeding=None, diff_repc = None):
        # e1_embedded_all = embedding
        batch_size = len(triplets)
        if pre_type =="all":
            e1_embedded_all = F.tanh(embedding)
        ...
        if partial_embeding is None:
            ...
            evlove = torch.mm(diff_repc, (pre_weight*embedding + (1-pre_weight)*his_emb).transpose(1, 0))
            x1 = x
            gate = self.gate(triplets[:, 0], triplets[:, 1])
            x = (torch.mul(gate, x) + torch.mul(-gate + 1, evlove))
        else:
            x = torch.mm(x, partial_embeding.transpose(1, 0))
            evlove = torch.mm(diff_repc, e1_embedded_all.transpose(1, 0))
            x1 = x
            gate = self.gate(triplets[:, 0], triplets[:, 1])
            x = (torch.mul(gate, x) + torch.mul(-gate + 1, evlove))
        return x, cl_x, x1
...
class ConvTransRTime(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rel, time_embs, triplets,  nodes_id=None, mode="train", negative_rate=0):
...
class ConvTransETime(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rel, time_embs, triplets, his_emb, pre_weight, pre_type,partial_embeding=None):
        #e1_embedded_all = F.tanh(embedding)
        batch_size = len(triplets)
        if pre_type == "all":
            e1_embedded_all = F.tanh(embedding)
        ...
        rel_embedded = emb_rel[triplets[:, 1]].unsqueeze(1)
        stacked_inputs = torch.cat([e1_embed, rel_embedded, emb_time_1, emb_time_2], 1)
        stacked_inputs = self.bn0(stacked_inputs)
