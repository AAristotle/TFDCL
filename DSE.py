...
class DataPreprocessor:
    ...
    def _build_path(self, *args):
        ...
        return os.path.join(THIS_DIR, 'data', self.dataset, 'text_emb', self.plm, *args)
    ...
    def load_id_data(self, fileNames=['train.txt', 'valid.txt', 'test.txt']):
        ...
        for fileName in fileNames:
            with open(os.path.join(THIS_DIR, 'data', self.dataset, fileName), 'r') as fr:
            # with open(os.path.join(THIS_DIR.parent, self.dataset, fileName), 'r') as fr:
            # with open(os.path.join('./data',self.dataset,fileName), 'r') as fr:
    ...
    def _load_id2ent_GDELT(self):
        ...
        with open(os.path.join(THIS_DIR, 'data', self.dataset, 'entity2id.txt'), 'r') as f:
    ...
    def _load_id2rel_GDELT(self):
        ...
        with open(os.path.join(THIS_DIR, 'data', self.dataset, 'relation2id.txt'), 'r') as f1, \
             open(os.path.join(THIS_DIR, 'data', self.dataset, 'CAMEO-eventcodes.txt'), 'r') as f2:
    ...
    def _load_id2ent_id2rel_default(self):
        ...
        # with open(os.path.join(THIS_DIR, 'data', self.dataset, 'relation2id.txt'), 'r') as f:
        #     id2rel = {int(line.split('\t')[1]): line.split('\t')[0] for line in f}
        with open(os.path.join(THIS_DIR, 'data', self.dataset, 'relation2id.txt'), 'r') as f:
            raw_id2rel = {int(line.split('\t')[1]): line.split('\t')[0] for line in f}

        num_rel = len(raw_id2rel)
        id2rel = raw_id2rel.copy()
        for rel_id, rel_name in raw_id2rel.items():
            id2rel[rel_id + num_rel] = rel_name + "_inverse"
...
class DSEncoder(nn.Module):
    ...
    def _load_pretrained_model(self):
        ...
        if self.model_type == 'bert':
            ...
            model_path = os.path.join(THIS_DIR, 'meta-llama/Meta-Llama-3.1-8B')
