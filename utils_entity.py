...
def load_all_answers_for_filter2(total_data, num_rel, rel_p=False):
    # store subjects for all (rel, object) queries and
    # objects for all (subject, rel) queries
    def add_relation(e1, e2, r, d):
        if not e1 in d:
    ...
    for line in total_data:
        s, r, o = line[: 3]
        if rel_p:
            add_relation(o, s, r, all_ans)
            add_relation(s, o, r + num_rel, all_ans)
        else:
            add_subject(o, s, r, all_ans, num_rel=num_rel)
            add_object(o, s, r, all_ans, num_rel=0)
    return all_ans
...
def load_all_answers_for_time_filter2(total_data, num_rels, num_nodes, rel_p=False):
    all_ans_list = []
    all_snap = split_by_time(total_data)
    for snap in all_snap:
        all_ans_t = load_all_answers_for_filter2(snap, num_rels, rel_p)
        all_ans_list.append(all_ans_t)
...
def load_data(dataset):
    if dataset in ['EG', 'IS', 'IR','ICEWS14','ICEWS14_REL','ICEWS05_15','ICEWS18','WIKI','GDELT','YAGO', 'ICEWS05-15']:
...
def build_sub_graph2(num_nodes, num_rels, triples, gpu):
