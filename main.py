...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, err_mat_list, err_inv_mat_list, query_times, mode):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        err_mat = sparse_mx_to_torch_sparse_tensor(err_mat_list[time_idx])
        err_inv_mat = sparse_mx_to_torch_sparse_tensor(err_inv_mat_list[time_idx])
        ...
        sequence = np.load(
            './data/{}/history_seq/h_r_seen_triple_{}.npy'.format(args.dataset, str(query_times[time_idx])),
            allow_pickle=True).tolist().toarray().reshape(test_triples_input.shape[0] * 2, args.max_len, 3) - 1
        ...
        nn = len(ent_ent_his_emb[query_time-1]) // 2

        test_triples, final_score = model.predict(que_pair, sub_snap, time_idx, history_glist, num_rels, static_graph, test_triples_input, time_idx, err_mat, sequence1, use_cuda, ent_ent_his_emb[query_time-1][:nn], ent_rel_his_emb[query_time-1][:nn])
        inv_test_triples, inv_final_score = model.predict(que_pair_inv, sub_snap_inv, time_idx, history_glist, num_rels, static_graph, test_triples_input_inv, time_idx, err_inv_mat, sequence2, use_cuda, ent_ent_his_emb[query_time-1][nn:], ent_rel_his_emb[query_time-1][nn:])
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    # model_state_file = './models/ICEWS14-len7-gpu0-lr0.001-0.03-0.9-True-all-200-uvrgcn-1745721773.3534644.pt'
    ...
    def create_model_diffu(args):
        ...
        return diffu_pre
    diffu_rec = create_model_diffu(args)    # create stat
    ...
    if args.test and os.path.exists(model_state_file):
        mrr_raw, mrr_filter= test(model,
                                ...
                                model_state_file, 
                                static_graph,
                                err_mat_list[len(train_list + valid_list):],
                                err_inv_mat_list[len(train_list + valid_list):],
                                test_times,
                                "test")
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                sequence = np.load('./data/{}/history_seq/h_r_seen_triple_{}.npy'.format(args.dataset, str(train_times[train_sample_num])), allow_pickle=True).tolist().toarray().reshape(output[0].shape[0] * 2,  args.max_len, 3) - 1
                sequence = torch.from_numpy(sequence).long().cuda() if use_cuda else torch.from_numpy(sequence).long()
                sequence1 = sequence[:len(sequence) // 2]
                sequence2 = sequence[len(sequence) // 2:]
                ...
                err_mat = sparse_mx_to_torch_sparse_tensor(err_mat_list[train_sample_num])
                err_inv_mat = sparse_mx_to_torch_sparse_tensor(err_inv_mat_list[train_sample_num])

                nn = len(ent_ent_his_emb[train_sample_num]) // 2
                for id in range(2):
                    if id % 2 == 0:
                        loss_e, loss_r, loss_static, loss_cl, loss_vocal, loss_diff = model.get_loss(que_pair, subg_snap, train_sample_num, history_glist, triples, static_graph, train_sample_num, sub_d_dict_er2e, use_cuda, err_mat, sequence1, True, ent_ent_his_emb[train_sample_num][:nn], ent_rel_his_emb[train_sample_num][:nn])#这里传入了两相同的值，但功能不同，代码能跑就行，后期再说
                    else:
                        loss_e, loss_r, loss_static, loss_cl, loss_vocal, loss_diff = model.get_loss(que_pair_inv, subg_snap_inv, train_sample_num, history_glist, inverse_triples, static_graph, train_sample_num, ob_d_dict_er2e, use_cuda, err_inv_mat, sequence2, True, ent_ent_his_emb[train_sample_num][nn:], ent_rel_his_emb[train_sample_num][nn:])

                    loss = loss_e + loss_r + loss_static + loss_cl + loss_vocal + loss_diff
                    # loss = 0.8 * loss_e + 0.2 * loss_r + 0.5 * loss_static
            ...
            if epoch and epoch % args.evaluate_every == 0:
                mrr_raw, mrr_filter = test(model, 
                                    ...
                                    model_state_file,
                                    static_graph,
                                    err_mat_list[len(train_list):],
                                    err_inv_mat_list[len(train_list):],
                                    valid_times,
                                    mode="train")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--hidden_size", type=int, default=400, help="number of hidden units")
    parser.add_argument("--easy_copy", type=float, default=0, help="if 0, print all outputs")
    parser.add_argument('--schedule_sampler_name', type=str, default='lossaware', help='Diffusion for t generation')
    parser.add_argument('--diffusion_steps', type=int, default=10, help='Diffusion step')
    ...
    parser.add_argument('--num_blocks_cross', type=int, default=1, help='Number of Transformer blocks')
    parser.add_argument("--max_len", type=int, default=128)
    parser.add_argument("--diffusion_max_len", type=int, default=64)
